...
One asset per source today (dependency manifests). Returns the candidates
themselves (not a file path) so downstream assets (enrich, validate,
load) receive them through Dagster's IO manager as a normal asset
dependency, rather than each stage re-reading a side-effect file.
"""
