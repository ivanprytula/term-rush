...
(extract) -> enriched_candidates (enrich) -> validated_candidates
(validate) -> loaded_candidates (load). Each asset returns its output
directly rather than writing a side-effect file, so Dagster passes data
downstream through the ordinary asset-dependency mechanism (parameter
name == upstream asset name) and the lineage graph reflects the real
data flow.
"""
